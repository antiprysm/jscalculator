function App() {
    ...
    const display = (symbol) => {
        ...
        if(expression.length >= 1 && /(\/|\.|\*|\-|\+)/.test(symbol) && /(\/|\.|\*|\-|\+)/.test(expression[expression.length-1])) {
            setExpression(expression)
        }
        if(expression.split(/(\/|\-|\+|\*)/).slice(-1).map(x => /\./.test(x) ? x.match(/\./g).length : 0).some(x => x >= 1) && symbol === '.') {
            setExpression(expression)
        }
    };  
    const calculate = () => {
    ...
    <div className="container">
        <div className="grid">
            <div onClick={display} className="dis" id="display">
                ...
                <div className="total" id="total" >{answer}</div>
            </div>
